...
const Templates = ({id, setActivePanel, activeMood, activeText, setActiveText}) => {
    ...
    <Panel id={id}>
      <div className={styles.root}>
        <div className={styles.header + " flex justify-between"}>
          <svg onClick={() => setActivePanel("ask")} className={styles.back}
          ...
          <div className={'px-[20px] py-[10px] bg-[#549853] text-white ml-[-40px]'}>Шаблоны</div>
          <div></div>
        </div>
        ...
        <div className={'flex flex-col justify-start w-full gap-[10px] pt-[20px]'}>
          {data.map((item) => (
            <div key={item.id}
                 onClick={() => setActiveText((prev) => prev + '\n' + item.title)}
                 className={'rounded-[20px] border border-[#549853] text-[#549853] px-[20px] py-[10px] cursor-pointer hover:bg-[#549853] hover:text-white'}>
              {item.title}
            </div>
          ))}
        </div>
